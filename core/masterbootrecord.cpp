...
MasterBootRecord::MasterBootRecord(long diskCapacity){
...
logic::Partition* MasterBootRecord::getPartition(int index){
    if(index >= MasterBootRecord::MAX_PARTITION){
        throw std::runtime_error("Index out of range");
    }
    if(nullptr == partitions_m[index]){
        throw std::runtime_error("Access to nullpointer");
    }
...
std::ostream& disk::operator<< (std::ostream& os, MasterBootRecord& mbr) {
    double percentage = 0;
